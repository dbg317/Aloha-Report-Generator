...
print(
        'Aloha Daily Status Report Generator V1.2.2 by Bernard'
        ...
        +'\n- Issues fixed on v1.3:'
        +'\n 1. add comment for tech N/A entry'
)
    ...
    for row in reader_list:
        if row[7] == 'FAIL' and row[16] == 'PASS' and row[6] != 'Discover':
            ...
            for i in fail_comment:
                if 'reader'in i or 'Reader' in i or 'pos' in i or 'POS' in i:
            ...
            l1 = get_other_failed_cards(row)
            if len(l1) == 0:
                report_file.write('No other DUT cards failed. ')
            else:
                for i in l1: report_file.write(i[1]+' '+i[2]+' '+'FPAN:'+i[3]+' '+'DPAN:'+i[4]+'; ')
            l2 = get_other_passed_cards(row)
            report_file.write('\nOther cards passed: ')
