...
print(
        'Aloha Daily Status Report Generator V1.2.1 by Bernard'
        +'\n- Compatible with PayApp V1.9.11 (stockholm)'
        ...
        +'\n- Issues fixed on v1.2:'
        +'\n 1. three digits DPAN/FPAN casing by 0 in the begging'
        +'\n 2. "No issue" section now will ask user if all cards has passed for stores partially tested'
        +'\n 3. removed leading/trailing white space from csv entries.'
        +'\n 4. report format fix'
        +'\n 5. drag and drop .csv file'
)
...
with open(file_name, newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        for i in range(0,38):
            row[i] = row[i].strip()
            if i == 17 or i == 21:  #append 0 to 3 digits DPAN/FPAN
                if len(row[i]) < 4:
                    row[i] = '0' + row[i]
        # print(row)
...
def no_issue():
    ...
    for item in index_list:  # find out if all duplicates have pass verdict
        ...
        if pass_counter == len(item):  # check if all cards pass, and total cards number
            if len(item) == card_numbers:
                for i in item:
                    no_issue_list.append(location_address_list[i])
                    break
            elif len(item) < card_numbers:
                print('\nNot all cards detected for "'+location_list[item[0]]+'" did all DUT cards passed\n')
                answer = input('y/n?\n')
                if answer == 'y':
                    print(answer)
                    for i in item:
...
def device_issue():  # can not loop the same iterator twice
    ...
    for row in reader_list:
        if row[7] == 'FAIL' and row[16] == 'PASS' and row[6] != 'Discover':
            ...
            report_file.write('Other cards passed: ')
            if len(l2) == 0:
                report_file.write('No other DUT cards passed. ')
            else:
